...
$almacenNum = mb_strtoupper(fString($obra->almacen ?? '109'));
...
$fecha = (isset($servicio) && $servicio->fechaProgramacion !== null) ? fFechaLarga($servicio->fechaProgramacion) : '';
...
$pdf->MultiCell(16, 3, "{$almacenNum}", 'B', 'C', 0, 1, '', '', true);
...
foreach($requisicion->detalles as $key => $detalle) {
	...
	$pu = isset($detalle['costo']) ? number_format($detalle['costo'], 2) : '0.00';
	...
	$pdf->MultiCell(18, $altoFila, "", 1, 'C', 0, 0, '', '', true, 0, false, true, $altoFila, 'M');
...
$justificacion = mb_strtoupper(fString("BANCO: {$gasto->banco} \n CUENTA: {$gasto->cuenta} \n CLABE: {$gasto->clave}") ?? 'NA');
