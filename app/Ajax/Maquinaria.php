...
class Maquinaria extends MaquinariaPolicy
{
    static protected $fillable = [
        'empresaId', 'numeroEconomico', 'numeroFactura', 'maquinariaTipoId', 'modeloId', 'year', 'descripcion', 'serie', 'colorId', 'estatusId', 'ubicacionId', 'almacenId', 'observaciones','obraId', 'fugas', 'transmision', 'sistema', 'motor'
    ];
    ...
    public function consultar($item = null, $valor = null) {
        ...
        if ( is_null($valor) ) {
            ...
            return Conexion::queryAll($this->bdName, "SELECT M.*, EM.nombreCorto AS 'empresas.nombreCorto', MT.descripcion AS 'maquinaria_tipos.descripcion', MO.descripcion AS 'modelos.descripcion', MA.descripcion AS 'marcas.descripcion', C.descripcion AS 'colores.descripcion', E.descripcion AS 'estatus.descripcion', U.descripcion AS 'ubicaciones.descripcion', A.descripcion AS 'almacenes.descripcion' FROM $this->tableName M INNER JOIN empresas EM ON M.empresaId = EM.id INNER JOIN maquinaria_tipos MT ON M.maquinariaTipoId = MT.id INNER JOIN modelos MO ON M.modeloId = MO.id INNER JOIN marcas MA ON MO.marcaId = MA.id LEFT JOIN colores C ON M.colorId = C.id INNER JOIN estatus E ON M.estatusId = E.id INNER JOIN ubicaciones U ON M.ubicacionId = U.id INNER JOIN almacenes A ON M.almacenId = A.id ORDER BY M.maquinariaTipoId, M.descripcion", $error);
    ...
    public function actualizar($datos) {
        ...
        $datos["fugas"] = $datos["fugas"] == "on" ? 1 : 0;
        $datos["transmision"] = $datos["transmision"] == "on" ? 1 : 0;
        $datos["sistema"] = $datos["sistema"] == "on" ? 1 : 0;
        $datos["motor"] = $datos["motor"] == "on" ? 1 : 0;
    ...
    public function guardarImagenes($detalle)
    {
        $respuesta = array();
        ...
        if ( isset($_FILES["images"]) ) {
            ...
            for ($i = 0; $i < $total; $i++) {
                ...
                $ruta = fRandomNameFile($directorio, '.jpg');
                $nombre = $file["name"];
                
                if ( move_uploaded_file($file["tmp_name"], $ruta) ) {
                    $datos = array();
                    $datos["maquinariaId"] = $this->id;
                    $datos["ruta"] = substr($ruta,5);
                    $datos["nombre"] = $nombre;
