...
class RequisicionArchivo
{
    ...
    public function consultarComprobantesDeRequisiciones()
    {
        $respuesta = Conexion::queryAll($this->bdName, "SELECT 
                                                            ...
                                                            RA.*
                                                        FROM 
                                                            requisicion_archivos RA 
                                                        INNER JOIN 
                                                            `ordencompra` OC 
                                                        ON
                                                            RA.ordenCompraId = OC.id
                                                        WHERE 
                                                            RA.proveedorId =  $this->proveedorId", $error);
        return $respuesta;
    }
    public function obtenerOrdenesCompraFaltanFactura() {
        ...
        return Conexion::queryAll($this->bdName, "SELECT 
                                                        ...
                                                        LEFT JOIN requisicion_archivos RA 
                                                            ON RA.ordenCompraId = OC.id 
                                                            AND RA.tipo = 3
                                                        WHERE P.id = $this->proveedorId
                                                        AND RA.id IS NULL
                                                         ORDER BY OC.fechaCreacion DESC", $error);
    }

    public function obtenerOrdenesCompraFaltanComprobantes() {
