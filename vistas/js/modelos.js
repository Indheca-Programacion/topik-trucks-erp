$(function(){
	...
	let tableList = document.getElementById('tablaModelos');

	// LLamar a la funcion fAjaxDataTable() para llenar el Listado
	if ( tableList != null ) fAjaxDataTable(rutaAjax+'app/Ajax/ModeloAjax.php', '#tablaModelos');

	// Confirmar la eliminación del Modelo
	$(tableList).on("click", "button.eliminar", function (e) {
		...
		Swal.fire({
			title: '¿Estás Seguro de querer eliminar este Modelo (Descripción: '+folio+') ?',
	...
	let elementMarcaId = $('#marcaId.select2.is-invalid');
	if ( elementMarcaId.length == 1 ) { 
