...
$pdf->MultiCell(15, 8, "NO.", 1, 'C', 1, 0, 5, '', true, 0, false, true, '7', 'M');
$pdf->MultiCell(25, 8, "EQUIPO", 1, 'C', 1, 0, '', '', true, 0, false, true, '7', 'M');
$pdf->MultiCell(30, 8, "MARCA", 1, 'C', 1, 0, '', '', true, 0, false, true, '7', 'M');
$pdf->MultiCell(30, 8, "MODELO", 1, 'C', 1, 0, '', '', true, 0, false, true, '7', 'M');
$pdf->MultiCell(25, 8, "NO. SERIE", 1, 'C', 1, 0, '', '', true, 0, false, true, '7', 'M');
$pdf->MultiCell(25, 8, "NO. ECONOMICO", 1, 'C', 1, 0, '', '', true, 0, false, true, '7', 'M');
$pdf->MultiCell(20, 8, "SERV.", 1, 'C', 1, 0, '', '', true, 0, false, true, '7', 'M');
$pdf->MultiCell(40, 8, "UBICACION", 1, 'C', 1, 0, '', '', true, 0, false, true, '7', 'M');
$pdf->MultiCell(30, 8, "FECHA", 1, 'C', 1, 0, '', '', true, 0, false, true, '7', 'M');
$pdf->MultiCell(40, 8, "TIPO DE MANT.", 1, 'C', 1, 1, '', '', true, 0, false, true, '7', 'M');
$pdf->SetFillColor(255, 255, 255); // Color de fondo

foreach ($programaciones as $key => $value) {
	$partida = $key + 1;
	$proximoServicio = $value['cantidadSiguienteServicio'] + $value["horoOdometroUltimo"];
	$equipo = mb_strtoupper(fString($value['maquinaria.equipo']));
	$marca = mb_strtoupper(fString($value['maquinaria.marca']));
	$modelo = mb_strtoupper(fString($value['maquinaria.modelo']));
	$serie = mb_strtoupper(fString($value['maquinaria.serie']));
	...
	$servicioTipo = mb_strtoupper(fString($value['servicio_tipos.descripcion']));
	$fecha = date('Y-m-d', strtotime('+1 week', strtotime(date('Y-m-d'))));
	$y_end = $pdf->GetY();

	$pdf->MultiCell(15, 8, "{$partida}", 1, 'C', 1, 0, 5, '', true, 0, false, true, '7', 'M');
	$pdf->MultiCell(25, 8, "{$equipo}", 1, 'C', 1, 0, '', '', true, 0, false, true, '7', 'M');
	$pdf->MultiCell(30, 8, "{$marca}", 1, 'C', 1, 0, '', '', true, 0, false, true, '7', 'M');
	$pdf->MultiCell(30, 8, "{$modelo}", 1, 'C', 1, 0, '', '', true, 0, false, true, '7', 'M');
	$pdf->MultiCell(25, 8, "{$serie}", 1, 'C', 1, 0, '', '', true, 0, false, true, '7', 'M');
	$pdf->MultiCell(25, 8, "{$numeroEconomico}", 1, 'C', 1, 0, '', '', true, 0, false, true, '7', 'M');
	$pdf->MultiCell(20, 8, "{$proximoServicio} {$value['unidades.nombreCorto']}", 1, 'C', 1, 0, '', '', true, 0, false, true, '7', 'M');
	$pdf->MultiCell(40, 8, "{$ubicacion}", 1, 'C', 1, 0, '', '', true, 0, false, true, '7', 'M');
	$pdf->MultiCell(30, 8, "", 1, 'C', 1, 0, '', '', true, 0, false, true, '7', 'M');
	$pdf->MultiCell(40, 8, "{$servicioTipo}", 1, 'C', 1, 1, '', '', true, 0, false, true, '7', 'M');
...
$ruta_pdf = $ruta_directoro.'/tmp/ProgramacionMantenimiento.pdf';
