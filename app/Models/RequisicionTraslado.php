...
if ( file_exists ( "app/Policies/RequisicionTrasladoPolicy.php" ) ) {
    require_once "app/Policies/RequisicionTrasladoPolicy.php";
} else {
    require_once "../Policies/RequisicionTrasladoPolicy.php";
}

use App\Conexion;
use PDO;
use App\Policies\RequisicionTrasladoPolicy;

class RequisicionTraslado extends RequisicionTrasladoPolicy
{
    static protected $fillable = [
        'trasladoId', 'numero', 'folio', 'servicioEstatusId', 'detalles', 'detalle_imagenes', 'comprobanteArchivos', 'ordenesArchivos', 'observacion', 'facturaArchivos', 'cotizacionArchivos', 'valeArchivos', 'partidasEliminadas', 'actualServicioEstatusId'
    ];
    ...
    protected $tableName = "requisicion_traslados";
    ...
    public function crear($datos) {
        ...
        $lastId = $this->consultarLastId($datos["trasladoId"]);

        if ( $lastId === false ) {

            $lastId = $this->consultarFolio($datos["trasladoId"]);
        }
        ...
        $arrayPDOParam = array();
        $arrayPDOParam["trasladoId"] = self::$type["trasladoId"];
    ...
    function insertarDetalles(array $arrayDetalles) {
        ...
        if ( $arrayDetalles ) {
            ...
            $insertarPDOParam = array();
            $insertarPDOParam["trasladoId"] = self::$type[$this->keyName];
