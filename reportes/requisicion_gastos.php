...
class MYPDF extends TCPDF {
	...
	public function Header() {
		...
		$this->MultiCell(25, 5, "", 1, 'C', 0, 0, '', '', true);
...
foreach($requisicion->detalles as $key => $detalle) {
	...
	$costo = mb_strtoupper($detalle['costo']);
...
$pdf->MultiCell(100, 5, "CUENTAS:", 0, '', 0, 1, '', '', true, 0, false, true, '5', 'M');
$pdf->MultiCell(100, 12, "{$justificacion}", 0, 'C', 0, 0, '5', '', true, 0, false, true, '12', 'M');
$pdf->Line(10, 247, 100, 247, false);
$pdf->MultiCell(100, 12, " BANCO: {$gasto->banco} \n CUENTA: {$gasto->cuenta} \n CLABE: {$gasto->clave}", 0, '', 0, 1, '', '', true, 0, false, true, '12', 'M');
...
$ubicacion = mb_strtoupper(fString($requisicion->ubicacion['descripcion'] ?? $obra->descripcion));
