...
class AlmacenAjax
{
	...
	public function agregar(){
        ...
        if ( usuarioAutenticado() ) {
			...
			if ( !Autorizacion::perfil($usuario, CONST_ADMIN) && !Autorizacion::permiso($usuario, "almacenes", "crear") ) {

	            $respuesta["error"] = true;
				$respuesta["errorMessage"] = "No está autorizado a crear nuevos Almacenes.";
		...
		if ( $almacen->consultar("descripcion", $this->descripcion) ) {
	        ...
	        if ( $almacen->crear($datos) ) {

	        	$respuesta["respuestaMessage"] = "El almacén fue creado correctamente.";

				// Si lo pudo crear, consultar el registro para obtener el Id en el Ajax
	        	$respuesta["respuesta"] = $almacen->consultar("descripcion", $this->descripcion);
...
if ( isset($_POST["nombreAlmacen"]) ) {
	...
	AGREGAR ALMACEN
	=============================================*/	
	$almacenAjax->token = $_POST["_token"];
	$almacenAjax->descripcion = $_POST["nombreAlmacen"];
	$almacenAjax->agregar();
