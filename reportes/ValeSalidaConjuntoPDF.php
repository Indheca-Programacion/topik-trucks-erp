...
class MYPDFValeSalida extends TCPDF {
    public $logo;
	public $empresaId;
	public $empresa;
	public $folio;
...
function generarPDFValeSalida($salidasInventario) {

    // create new PDF document
    $pdf = new MYPDFValeSalida(PDF_PAGE_ORIENTATION, PDF_UNIT, PDF_PAGE_FORMAT, true, 'UTF-8', false);
    ...
    foreach ($salidasInventario as $salidaInventario) {
        
        $pdf->folio = $salidaInventario["id"];
        ...
        $modeloInventarioSalida = New \App\Models\InventarioSalida;
        $modeloAlmacen = New \App\Models\Almacen;
        $modeloUsuario = New \App\Models\Usuario;

        // CONSTULTAR PARTIDAS DE LA ENTRADA
        $detalles = $modeloInventarioSalida->consultarPartidaSalidaPorId($salidaInventario["id"]);
        // CONSULTAR ALMACEN DE LA ENTRADA
        $modeloAlmacen->consultar(null, $salidaInventario["almacenId"]);

        // USUARIO QUE RECIBIO
        $modeloUsuario->consultar(null, $salidaInventario["usuarioRecibioId"]);
        ...
        $usuarioFirmaRecibio = $_SERVER['DOCUMENT_ROOT'] . $salidaInventario["firma"];

        // USUARIO QUE ENTREGO
        $modeloUsuario->consultar(null, $salidaInventario["usuarioEntregoId"]);
        $usuarioNombreEntrego = mb_strtoupper($modeloUsuario->nombre . ' ' . $modeloUsuario->apellidoPaterno);
        
        $usuarioFirmaEntrego =  $_SERVER['DOCUMENT_ROOT'] . $modeloUsuario->firma;
     
        $pdf->AddPage();

        $pdf->Ln(8); // Salto de Línea

        $ordenCompra = mb_strtoupper(fString($salidaInventario["ordenCompra"]));
        $nombreAlmacen = mb_strtoupper(fString($modeloAlmacen->descripcion));
        $fechaEntrega = fFechaLarga($salidaInventario["fechaSalida"]);
    
        $pdf->SetFont('helvetica', 'B', 8); // Fuente, Tipo y Tamaño

        $pdf->MultiCell(40, 5, "NOMBRE QUIEN RECIBE:", 0, 'R', 0, 0, 5, 30, true);
        $pdf->MultiCell(80, 5, "{$usuarioNombreRecibio}", 'B', '', 0, 0, '', '', true);
        ...
        $pdf->SetX(12);
        $observaciones = mb_strtoupper($salidaInventario["observaciones"]);
        ...
        if ($usuarioFirmaEntrego != "/var/www/html/") {
        ...
        if ($usuarioFirmaRecibio != "/var/www/html/") {
            $pdf->Ln(40); // Salto de Línea
    ...
    $ruta = __DIR__ . '/vale-salida/vale-salida.pdf';  
    $pdf->Output($ruta, 'F'); 
