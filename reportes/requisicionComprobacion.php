...
$pdf->folio = mb_strtoupper(fString($comprobacion->folio));
$pdf->fechaCreacion = $comprobacion->fechaCreacion;
$pdf->maquinaria = isset($comprobacion->maquinaria) ? $comprobacion->maquinaria : 'NA' ;
$pdf->mantenimientoTipo = isset($comprobacion->mantenimientoTipo) ? $comprobacion->mantenimientoTipo : 'NA' ;
...
foreach($comprobacion->detalles as $key => $detalle) {
	...
	$y_start = $pdf->GetY();
	if ( $y_start > 223 && $partida == count($comprobacion->detalles) ) {
		$pdf->AddPage();
	...
	$num_detalle = count($comprobacion->detalles);
...
$justificacion = mb_strtoupper(fString($comprobacion->justificacion ?? 'NA'));
...
$pdf->MultiCell(66, 5, "VB COMPROBACIONES", 0, 'C', 0, 0, '', '', true);
