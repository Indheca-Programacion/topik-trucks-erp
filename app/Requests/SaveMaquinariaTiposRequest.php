...
class SaveMaquinariaTiposRequest extends Request
{
    static public function rules($id)
    {
        if ( self::method() === 'POST' ) {
            $rules = [ 'descripcion' => 'required|string|max:60|unique:'.CONST_BD_APP.'.maquinaria_tipos', 
                       'nombreCorto' => 'required|string|max:20|unique:'.CONST_BD_APP.'.maquinaria_tipos' ];
        } else {
            $rules = [ 'descripcion' => 'required|string|max:60|unique:'.CONST_BD_APP.'.maquinaria_tipos:id:' . $id,
                       'nombreCorto' => 'required|string|max:20|unique:'.CONST_BD_APP.'.maquinaria_tipos:id:' . $id ];
        }
    ...
    static public function messages()
    {
        return [
            ...
            'nombreCorto.required' => 'El nombre corto del Tipo de Maquinaria es obligatorio.',
    ...
    static public function validated($id = null) {
        return self::validating(MaquinariaTipo::fillable(), self::rules($id), self::messages());
    }
}
