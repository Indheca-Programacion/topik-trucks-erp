...
// $fecha_inicio = fFechaLarga($gastos->fecha_envio);
...
foreach($detallesGastos as $key => $detalle) {
	...
	$observaciones = mb_strtoupper($detalle["observaciones"]);
	...
	$pdf->MultiCell(45, 0, "{$observaciones}", 1, 'C', 0, 1, 245, '', true, 0);
	$y_end = $pdf->GetY();
	...
	$pdf->MultiCell(30, $altoFila, "{$tipoGasto}", 1, 'C', 0, 0, '', '', true, 0, false, true, $altoFila, 'M');
	$pdf->MultiCell(40, $altoFila, "{$proveedor}", 1, 'C', 0, 0, '', '', true, 0, false, true, $altoFila, 'M');
	...
	$pdf->MultiCell(40, $altoFila, "{$solicito}", 1, 'C', 0, 0, '', '', true, 0, false, true, $altoFila, 'M');
	$pdf->MultiCell(30, $altoFila, "{$obraDescripcion}", 1, 'C', 0, 0, '', '', true, 0, false, true, $altoFila, 'M');
...
if ( !is_null($usuario->firma) ) {
	...
	$pdf->Image('../../' . $usuario->firma, 55, $y+15, 70, 0, $extension, '', '', false, 300, '', false, false, 0, 'CT', false, false);
}
$y = $pdf->getY()+5;
...
$pdf->Output("/var/www/html/control-mantenimiento/reportes/tmp/GastosDeducibles.pdf", 'F');
