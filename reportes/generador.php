...
foreach ($maquinariasPorEmpresa as $key => $maquinaria) {
	...
	$pdf->MultiCell(8, 21, "DIA PARCIAL", 1, 'C', 1, 0, '', '', true, 0, false, true, '7', 'M');
	$pdf->SetTextColor(0, 0, 0);
	$pdf->SetFillColor(221, 244, 245);
	...
	foreach ($maquinaria["maquinarias"] as $key => $value) {
		...
		$diaParcialCount = count($diaParcial);

		$totalDias = $efectivos+$fallasMecanicas+$parosOper+$climaCount+$diaParcialCount;
		$dm = 0;
		if ($totalDias !== 0) {
		...
		$pdf->MultiCell(8, 7, "{$diaParcialCount}", 1, 'C', 0, 0, '', '', true, 0, false, true, '7', 'M');
		$pdf->MultiCell(9, 7, "{$dm} %", 1, 'C', 0, 1, '', '', true, 0, false, true, '7', 'M');	
	
		if ( $y_end > 160 && $partida == count($maquinaria["maquinarias"]) ) {
			$pdf->AddPage();
		...
		$TotalDiaParcial += $diaParcialCount;
	}
	...
	if ( !is_null($elaboroFirma) ) {
		...
		$pdf->Image($elaboroFirma, 25, $y_nueva+10, 70, 0, $extension, '', '', false, 300, '', false, false, 0, 'CT', false, false);
	}
	...
	if ( !is_null($supervisoFirma) ) {
		...
		$pdf->Image($supervisoFirma, 205, $y_nueva+10, 70, 0, $extension, '', '', false, 300, '', false, false, 0, 'CT', false, false);
	}
	...
	if ( !is_null($autorizoFirma) ) {
		...
		$pdf->Image($autorizoFirma, 120, $y_nueva+10, 70, 0, $extension, '', '', false, 300, '', false, false, 0, 'CT', false, false);
	}

	$pdf->MultiCell(60, 6, "{$elaboro}", 'T', 'C', 1, 0, '', $y_nueva + 25, true, 0, false, true, '7', 'M');
	$pdf->MultiCell(60, 6, "{$superintendente}", 'T', 'C', 1, 0, 120, $y_nueva + 25, true, 0, false, true, '7', 'M');
	$pdf->MultiCell(60, 6, "{$superviso}", 'T', 'C', 1, 1, 210, $y_nueva + 25, true, 0, false, true, '7', 'M');
	$pdf->MultiCell(60, 7, "ELABORO", 0, 'C', 1, 0, 15, $y_nueva + 32, true, 0, false, true, '7', 'M');
	$pdf->MultiCell(20, 7, "SUPERVISO", 0, 'C', 1, 0, 140, $y_nueva + 32, true, 0, false, true, '7', 'M');
	$pdf->MultiCell(28, 7, "SUPERINTENDENTE", 0, 'C', 1, 0, 225, $y_nueva + 32, true, 0, false, true, '7', 'M');
