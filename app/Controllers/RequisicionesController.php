...
class RequisicionesController
{
    public function index() 
    {
        ...
        // Validar si el link contiene ?verPendientesAlmacen al final
        $arrayPuestos = array();
        $fechaActual = fFechaLarga(date('Y-m-d', strtotime('+1 days')));
        // Calcular la fecha de tres meses
        $fechaInicio = fFechaLarga(date('Y-m-d', strtotime('-3 months')));
        $estatusIdDefault = 0;
        if (strpos($_SERVER['REQUEST_URI'], '?verPendientesAlmacen') !== false) {
            $usuario = New Usuario;
            $usuario->consultar("usuario", usuarioAutenticado()["usuario"]);
            // var_dump($usuario->consultarPerfil(null, 7)); // 7 = Almacén
            require_once "app/Models/Puesto.php";
            $puesto = New \App\Models\Puesto;
            if($usuario->consultarPerfil(null, 7)) {
                $estatusIdDefault = 9; // 3 = Pendiente por Almacén
                $puestos = $puesto->consultarPuesto("id_usuario", $usuario->id);
                foreach ($puestos as $key => $value) {
                    if ( $value["puesto_usuario"] == "almacenista")
                    $arrayPuestos[] = $value["id_zona"];
                }
    ...
    {
        $requisicion = New Requisicion;
        Autorizacion::authorize('create', $requisicion);
        ...
        if ( $servicio->consultar(null , $id) ) {
            ...
            // if ( !$permitirModificarEstatus ) {
                ...
                // array_push($servicioStatus, $servicioEstatus->consultar('descripcion', 'proceso'));
    ...
    {    
        ...
        if ( $respuesta ) {
            ...
            if ( isset($request['soporteArchivos']) )
                array_push($uploadDocumentos, [
                    'id' => 6,
                    'tipoDocumento' => 'Soporte',
                    'documentos' => $request['soporteArchivos']['name']
                ]);
    ...
    public function edit($id)
    {
        ...
        if ( $requisicion->consultar(null , $id) ) {
            ...
            $mensajeRequisiciones = New \App\Models\MensajeRequisicion;
            $mensajeRequisiciones = $mensajeRequisiciones->consultar(null,$id);
            ...
            $traslado->consultar('servicio', $requisicion->servicioId);
    ...
    public function update($id)
    {
        ...
        if ( $request["servicioEstatusId"] != $request["actualServicioEstatusId"] && in_array($request["servicioEstatusId"], [5, 14]) && (empty($request['comprobanteArchivos']['name']))) {
    ...
    public function download($id, $tipo)
    {
        ...
        if ( $requisicion->consultar(null , $id) ) {
            ...
            elseif ( $tipo == 6 ) $archivos = $requisicion->soportes;
            else $archivos = array();
